...
class ScorecardTestSpec extends PlaySpec {
  ...
  "The Scorecard class" must {
    ...
    "update the scorecard when a criteria is set with an override for a measure" in {
      ...
      val scorecard = Scorecard().addScore("TEST", "TEST", HEDISRule.meetDemographic, true)
      ...
      val s2 = scorecard.addScore("TEST", "TEST", HEDISRule.eligible, true)
      ...
      val s3 = s2.addScore("TEST", "TEST", HEDISRule.excluded, true)
      ...
      val s4 = s2.addScore("TEST", "TEST", HEDISRule.meetMeasure, true)
    ...
    "update the scorecard for a unknown criteria throws and exception" in {
      ...
      a[NickelException] should be thrownBy {
        Scorecard().addScore("TEST", "TEST", "unknown", true)
      }
    }
    
    "update the scorecard when a criteria is set with a list of claims" in {
      
      val s1 = Scorecard().addScore("TEST", "TEST", HEDISRule.eligible, "predicate 1", List())
      ...
      val s2 = s1.addScore("TEST", "TEST", HEDISRule.eligible, "predicate 1", List(MedClaim("c", "p", "f", "l", "p", "f", "l", dos, dos)))
      s2.isPatientEligible("TEST") mustBe true
    }
  }
}