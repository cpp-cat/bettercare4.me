...
class CIS_MMR_RuleTestSpec extends PlaySpec {
  ...
  "The CIS_MMR_Rule class representing Measles-Mumps-Rubella (MMR) Vaccine HEDIS rule" must {
    ...
    "validate patient that meet the measure criteria" in {
      ...
      for (i <- 1 to 20) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(CIS_MMR.name, "F", new LocalDate(2012, 9, 12).toDateTimeAtStartOfDay(), 100, 0, 100)
    ...
    "validate patient that does not meet the measure criteria and is not excluded" in {
      ...
      for (i <- 1 to 20) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(CIS_MMR.name, "M", new LocalDate(2012, 9, 12).toDateTimeAtStartOfDay(), 100, 0, 0)
        val scorecard = rule.scoreRule(Scorecard(), patient, patientHistory)
