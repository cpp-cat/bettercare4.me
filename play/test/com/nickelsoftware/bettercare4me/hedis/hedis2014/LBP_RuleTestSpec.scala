...
class LBP_RuleTestSpec extends PlaySpec {
  ...
  "The LBP_Rule class representing Low Back Pain Imaging Studies HEDIS rule" must {
    ...
    "validate patient that meet the measure criteria" in {
      ...
      for (i <- 1 to 25) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(LBP.name, "M", new LocalDate(1972, 6, 1).toDateTimeAtStartOfDay(), 100, 0, 100)
    ...
    "validate patient that is excluded from the criteria" in {
      ...
      for (i <- 1 to 25) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(LBP.name, "F", new LocalDate(1972, 6, 1).toDateTimeAtStartOfDay(), 100, 100, 0)
        val scorecard = rule.scoreRule(Scorecard(), patient, patientHistory)
    ...
    "validate patient that does not meet the measure criteria and is not excluded" in {
      ...
      for (i <- 1 to 25) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(LBP.name, "M", new LocalDate(1972, 6, 1).toDateTimeAtStartOfDay(), 100, 0, 0)
        val scorecard = rule.scoreRule(Scorecard(), patient, patientHistory)
