...
class CIS_HiB_RuleTestSpec extends PlaySpec with OneAppPerSuite {

  "The CIS_HiB_Rule class representing Hepatitis B Vaccine HEDIS rule" must {

    "validate patient that meet the measure criteria" in {

      for (i <- 1 to 20) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(CIS_HiB.name, "F", new LocalDate(2012, 9, 12).toDateTimeAtStartOfDay(), 100, 0, 100)
    ...
    "validate patient that does not meet the measure criteria and is not excluded" in {
      ...
      for (i <- 1 to 20) {
        val (patient, patientHistory, rule) = HEDISRulesTestSpec.setupTest(CIS_HiB.name, "M", new LocalDate(2012, 9, 12).toDateTimeAtStartOfDay(), 100, 0, 0)
        val scorecard = rule.scoreRule(Scorecard(), patient, patientHistory)
