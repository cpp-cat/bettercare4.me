...
class CDCLDLCRuleTestSpec extends PlaySpec with OneAppPerSuite {
  
    "The CDCLDLCRule class representing Diabetes Lipid Test HEDIS rule" must {
    
    "validate patient that meet the measure criteria" in {

      val (patient, patientHistory, rule) = CDCMeetCriteriaTest.meetCriteria(CDC_LDL_C.name, true)
    ...
    "validate patient that does not meet the measure criteria and is not excluded" in {
      ...
      val (patient, patientHistory, rule) = CDCMeetCriteriaTest.meetCriteria(CDC_LDL_C.name, false)
      rule.isPatientMeetDemographic(patient) mustBe true
      rule.isPatientEligible(patient, patientHistory) mustBe true
      rule.isPatientExcluded(patient, patientHistory) mustBe false
