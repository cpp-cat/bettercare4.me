...
- Added simple pagination to ruleScorecard.scala.html view and associated rule_scorecard table. 
  Pagination using patient_id range. Outstanding issues:
      o List of patients are in no particular order. Ordered by name across the pages would be desired.
      o Cannot jump to a future page, e.g., cannot jump from page 1 to page 5 since we don't know the first
        patient_id of page 5 ahead of time.
  Alternate and improved solution would be to create a rule_scorecard_paginated table:
      o Add patient_name collumn for patient full name as a clustering collumn to rule_scorecard. 
        The primary key would now be: PRIMARY KEY (rule_name, hedis_date, patient_name, patient_id)
      o Once the rule_scorecard populated, load the rule_scorecard_paginated table with primary key:
        PRIMARY KEY (rule_name, hedis_date, page, patient_name, patient_id) with only 20 records for each page.
      o Client-side code (views) would setup pages of multiple of 20 records.
  Potential outstanding issue:
      o Updating rule_scorecard table may require to rebuid rule_scorecard_paginated, since rule_scorecard_paginated
